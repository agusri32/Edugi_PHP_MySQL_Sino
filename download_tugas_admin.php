...
if($_GET['mode']=='delete'){
	...
	if($query){
		?><script language="javascript">document.location.href="?page=download_tugas_admin&status=2";</script><?php
	}
}
?>
...
<tr>
	...
	$view=mysqli_query($link,"select * from tbl_upload_tugas upload, data_guru guru, data_siswa siswa, setup_matapelajaran matkul where upload.id_guru=guru.id_guru and upload.id_siswa=siswa.id_siswa and upload.id_matapelajaran=matkul.id_matapelajaran order by tgl_upload asc");
	...
	<tr>
		...
		<a href="?page=download_tugas_admin&mode=delete&id_tugas=<?php echo $row['id_tugas'];?>&nama_file=<?php echo $row['nama_file']; ?>" onclick="return confirm('Apakah Anda yakin?')" title="Delete" class="icon-2 info-tooltip"></a>            
		</td>
	</tr>
	<?php
	}
